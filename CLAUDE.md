...
- **Secretos:** fuera del repo (user-secrets); `appsettings.json` solo con placeholders. ⚠️ **La contraseña de Supabase y la API key de Gemini NO están rotadas** (confirmado por el dueño el 2026-07-23; este archivo afirmaba lo contrario). Siguen **vivas** y aparecen en claro en el historial del repo **privado** (`archive-privado`, commits `29a3092`/`5cb601d`/`6151282`). **El repo público (`origin`) está limpio**: verificado el 2026-07-23, sus únicos 2 commits sobre `appsettings.json` traen placeholders y no hay ninguna `AIzaSy` en todo su historial. O sea: la exposición exige acceso al repo privado, no es descargable por cualquiera. El dueño lo pospuso hasta el lanzamiento en Play Store (decisión suya, no la reabras). **Footgun:** la rama local `private-archive` sí contiene esos commits — nunca la pushees a `origin`. (El secreto JWT sí está rotado: 2026-07-02 y otra vez en el cutover a Heroku.)
