...
public class RoundedBar : ContentView
{
    public static readonly BindableProperty ProgressProperty =
        BindableProperty.Create(nameof(Progress), typeof(double), typeof(RoundedBar), 0.0, propertyChanged: OnProgressChanged);
    ...
    // T31: progreso MOSTRADO (el tween lo interpola hacia Progress; layout/altura lo fijan en seco).
    ...
    public RoundedBar()
    {
        ...
        Content = _track;
        Apply();
        _track.SizeChanged += (_, _) => { _shown = Math.Clamp(Progress, 0, 1); UpdateFill(); };
    }
    ...
    private void Apply()
    {
        ...
        _shown = Math.Clamp(Progress, 0, 1);
        UpdateFill();
    }
    ...
    {
        var w = _track.Width;
        _fill.WidthRequest = w <= 0 ? 0 : w * Math.Clamp(_shown, 0, 1);
    }

    // T31: ver llenarse la barra ES la recompensa — tween en vez de salto seco.
    private void AnimateProgress(double from, double to)
    {
        this.AbortAnimation("rbProgress");
        if (_track.Width <= 0) { _shown = Math.Clamp(to, 0, 1); return; } // aún sin layout: SizeChanged pinta en seco
        new Animation(v => { _shown = v; UpdateFill(); }, Math.Clamp(from, 0, 1), Math.Clamp(to, 0, 1), Easing.CubicOut)
            .Commit(this, "rbProgress", 16, 450);
    }

    private static void OnVisualChanged(BindableObject b, object o, object n) => ((RoundedBar)b).Apply();
    private static void OnProgressChanged(BindableObject b, object o, object n) => ((RoundedBar)b).AnimateProgress((double)o, (double)n);
